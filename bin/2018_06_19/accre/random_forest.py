...
DATA_FILE = "full.tsv" # Name of data file to process (full or test)
...
def test_model(model, x_df, y_df):
    ...
    x_resampled_df, y_resampled_df = oversampling.fit_sample(x_df, y_df)
    
    # Perform k-fold cross validation on the random forest model.
    cvs = cross_val_score(model, x_resampled_df, y_resampled_df, cv = CROSS_VAL)
...
def create_predictions(model, x_df, y_df):
    ...
    oversampling = over_sampling.RandomOverSampler()
    x_resampled_df, y_resampled_df = oversampling.fit_sample(x_train_df, y_train_df)
    
    # Train the model on the training data
    rfc.fit(x_resampled_df, y_resampled_df)
