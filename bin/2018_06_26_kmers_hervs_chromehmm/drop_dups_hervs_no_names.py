...
# by looking at chromosome, start, end. Stores the new
# data frame with these three columns to 
...
WRITE_FILE = "no_dups_hervs_no_names.tsv" # Name of file to write to
...
def main():
    ...
    new_herv_df = pd.DataFrame(columns = ["chr", "start", "end"])
    ...
    # Save to file
    no_dups_df.to_csv(DIRECTORY + "data/" + DATE + WRITE_FILE, sep = '\t', index = False, header = False)
