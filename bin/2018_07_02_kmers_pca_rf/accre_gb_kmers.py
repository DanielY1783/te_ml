...
def cross_validate(model, x, y):
    ...
    cvs_df.to_csv((DIRECTORY + "results/" + DATE_DIR + "gb_cross_val_scores.csv"), 
                  sep = '\t', index = False)
...
def predictions(model, x, y):
    ...
    cm_df.to_csv((DIRECTORY + "results/" + DATE_DIR + "gb_confusion_matrix.tsv"), sep = '\t')
    
    # Create a file to store metrics.
    with open((DIRECTORY + "results/" + DATE_DIR + "gb_metrics.txt"), "w+") as metrics_file:
