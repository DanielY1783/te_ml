...
# accre version of a support classifier run on the full HERVS kmers feature matrix at:
...
def main():
    ...
    # Test random forest model using cross validation.
    cross_validate(model = svc, x = x_df, y = y_df)
    
    # Generate predictions and get metrics.
    predictions(model = svc, x = x_df, y = y_df)
